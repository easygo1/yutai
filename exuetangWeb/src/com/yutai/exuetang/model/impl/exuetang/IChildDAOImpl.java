...
public class IChildDAOImpl implements IChildDAO {
	...
	public boolean addChild(Child child) {
		...
		// 向表中加入该用户
		sql = "insert into child(child_birthday,child_name,child_photo,child_sex,child_school_class_name,child_home_province,child_home_city,child_home_county,child_home_address,child_dream,child_hobby,child_trainplan,child_remarks) values"
				+ "(?,?,?,?,?,?,?,?,?,?,?,?,?);";
		try {
			...
			statement.setString(5, child.getChild_school_class_name());
			statement.setString(6, child.getChild_home_province());
			statement.setString(7, child.getChild_home_city());
			statement.setString(8, child.getChild_home_county());
			statement.setString(9, child.getChild_home_address());
			statement.setString(10, child.getChild_dream());
			statement.setString(11, child.getChild_hobby());
			statement.setString(12, child.getChild_trainplan());
			statement.setString(13, child.getChild_remarks());
		...
		try {
			...
			while (resultSet.next()) {
				...
				child=new Child(child_id1,user_id,child_birthday,
						...
						child_dream,child_hobby,child_trainplan,child_remarks);
	...
	public List<Child> selectAllChild() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				String child_trainplan=resultSet.getString(14);
				String child_remarks=resultSet.getString(15);
				child=new Child(child_id,user_id,child_birthday,
						child_name, child_photo, child_sex,
						child_school_class_name,child_home_province,
						child_home_city,child_home_county,child_home_address,
						child_dream,child_hobby,child_trainplan,child_remarks);
